  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 13, fontWeight: 900, color: "#333" }}>
        ...
        {TABS_BIB.map((t) => (
          <div
            ...
            style={{
              ...
              padding: "6px 10px",
              borderRadius: 6,
              border: `1.5px solid ${tab === t.id ? "#99bbcc" : "#ddd"}`,
              background: tab === t.id ? "#dceaf4" : "white",
              minWidth: 44,
            }}
          >
            <span
                ...
                {T12.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      {alt ? `${r}/${alt}` : r}{" "}
                      <span
                        style={{ fontSize: 9, color: "#aaa", fontWeight: 400 }}
                      >
                        {suf.slice(0, 3)}
                      </span>
                    </div>
...
const SC = {
  INTRO: { bg: "#eee", bd: "#bbb", lc: "#888" },
  ESTROFA: { bg: "#ddeedd", bd: "#99bb99", lc: "#557755" },
  ESTRIBILLO: { bg: "#f2e0e0", bd: "#ccaaaa", lc: "#996666" },
  PUENTE: { bg: "#dceaf4", bd: "#99bbcc", lc: "#557799" },
  OUTRO: { bg: "#eee", bd: "#bbb", lc: "#888" },
  VARIACION: { bg: "#f5f2cc", bd: "#ccbb66", lc: "#887733" },
};
...
const pcs = (a) => {
  const m = (a || "").match(/^([A-G][#b]?)(.*)$/);
  return m ? { root: m[1], sfx: m[2] } : { root: a || "", sfx: "" };
};
...
function HojaAcordes({ nombres, onBiblioteca }) {
  return (
    <div style={{ marginTop: 40, paddingTop: 20, borderTop: "1px solid #ddd" }}>
      <div
        style={{ fontSize: 9, fontWeight: 700, color: "#888", marginBottom: 8 }}
      >
...
function BeatEdit({ beat, onChange, onRemove, canRemove, bd, zoom }) {
  ...
  return (
    ...
    >
      <input
        value={beat.chord || ""}
        onChange={(e) => onChange({ ...beat, chord: e.target.value })}
        placeholder="acorde"
        style={{
          fontSize: 20 * z,
          fontWeight: 900,
          border: "none",
          borderBottom: `1.5px solid ${bd}99`,
      ...
      <input
        value={beat.note || ""}
        onChange={(e) => onChange({ ...beat, note: e.target.value })}
        placeholder="nota…"
        style={{
          fontSize: 10 * z,
          border: "none",
...
function CompasEdit({
  ...
  const upd = (bi, nb) => {
    const a = [...beats];
    a[bi] = nb;
    onChange({ ...compas, beats: a });
  };
  const add = () => {
    if (beats.length < 4)
      onChange({ ...compas, beats: [...beats, { chord: "", note: "" }] });
  };
  const rem = (bi) => {
    if (beats.length > 1) {
      const a = [...beats];
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRight: isLast ? "none" : `2px solid ${bd}66`,
        position: "relative",
        background: isFocused ? "#fffde7" : bg,
        outline: isFocused ? `2px solid #ccbb66` : "none",
    ...
    >
            ...
            <BeatEdit
              ...
              onChange={(nb) => {
                const orig = tAcorde(
                  nb.chord || "",
                  (12 - semi) % 12,
                  usarB(raizDeStr(nb.chord))
                );
                upd(bi, { ...nb, chord: orig });
              }}
              onRemove={() => rem(bi)}
              canRemove={beats.length > 1}
            />
          );
        })}
      </div>
      ...
      {conLetra && (
        <input
          value={compas.lyric || ""}
          onChange={(e) => onChange({ ...compas, lyric: e.target.value })}
          placeholder="letra…"
          style={{
            fontSize: 13 * z,
            border: "none",
            borderTop: `1px dashed ${bd}55`,
            background: `${bd}11`,
            width: "100%",
            padding: "2px 4px",
            outline: "none",
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        {beats.map((beat, bi) => {
          ...
          return (
            <div
              key={bi}
              onClick={() => cT && onClickAcorde(cT)}
              style={{
                width: `${widthPct}%`,
                flexShrink: 0,
                boxSizing: "border-box",
                borderRight: bi === nB - 1 ? "none" : `1px dashed ${bd}55`,
                padding: "3px 3px 2px",
                display: "flex",
                flexDirection: "column",
                alignItems: "flex-start",
                ...
                borderRadius: 3,
                position: "relative",
                zIndex: emptyAfter > 0 ? 2 : 1,
              }}
              onMouseEnter={(e) =>
                (e.currentTarget.style.background = bd + "33")
              }
              onMouseLeave={(e) =>
            ...
            >
              ...
              >
                ...
                {sfx && (
                  <span
                    style={{ fontSize: sfxFs, color: "#111", lineHeight: 1 }}
                  >
                    {sfx}
                  </span>
                )}
...
function BuscadorCancion({ onCargar }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 10, fontWeight: 700, color: "#557799" }}>
          🔍 BUSCAR CANCIÓN
        </span>
      </div>
      ...
      {error && (
        <div style={{ marginTop: 6, fontSize: 11, color: "#cc3333" }}>
          {error}
        </div>
      )}
      {loading && (
...
const INIT = {
  ...
  artista: "Joseph Kosma / Jacques Prévert",
  compas: "4/4",
  secciones: [
    {
      label: "INTRO",
    ...
    },
    {
      ...
      compases: [
        ...
        {
          beats: [{ chord: "Am7", note: "ritard." }],
          lyric: "I used to hold.",
        },
      ],
    },
    {
      ...
      compases: [
        ...
        {
          beats: [{ chord: "Am7", note: "rit. al fine" }],
          lyric: "start to fall.",
        },
      ],
    },
...
export default function App() {
  ...
  const acordesUnicos = [
    ...new Set(
      data.secciones.flatMap((s) =>
        s.compases.flatMap((c) =>
          c.beats.map((bt) => bt.chord || "").filter(Boolean)
        )
      )
    ),
  ].map((a) => a.split("/")[0]);
  ...
  const aplicarTono = (str) => {
    ...
    if (r && idxN(r) !== -1) {
      setSemi((((idxN(r) - idxN(R0)) % 12) + 12) % 12);
      setTono(r + (str.replace(/^[A-G][#b]?/, "") || S0));
    } else setTono(raiz + S0);
  };
  const btnSemi = (d) => {
    const s = (((semi + d) % 12) + 12) % 12;
    setSemi(s);
    setTono(mover(R0, s, false) + S0);
  };

  const updC = (si, ci, nc) => {
    const ns = [...data.secciones];
    ns[si] = { ...ns[si], compases: [...ns[si].compases] };
    ns[si].compases[ci] = nc;
    setData({ ...data, secciones: ns });
  };
  const updL = (si, v) => {
    const ns = [...data.secciones];
    ns[si] = { ...ns[si], label: v };
  ...
  const remC = (si) => {
    const ns = [...data.secciones];
    const c = [...ns[si].compases];
    if (c.length <= 1) return;
    c.pop();
    ns[si] = { ...ns[si], compases: c };
    setData({ ...data, secciones: ns });
  };
  const addS = () =>
    setData({
  ...
  if (vista === "biblioteca")
    return <Biblioteca onVolver={() => setVista("partitura")} />;
    ...
    >
      ...
      >
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          ...
          {capo > 0 && (
            <span style={{ fontSize: 10, color: "#a07800" }}>
              · capo {capo}
            </span>
          )}
        ...
        <div style={{ textAlign: "right" }}>
          {edit ? (
            ...
            <>
              ...
              <div style={{ fontSize: 13, color: "#777", lineHeight: 1 }}>
                {data.artista}
              </div>
            </>
          )}
      ...
      >
        <button
          ...
          style={{
            ...
            padding: "3px 12px",
            borderRadius: 4,
            border: `1.5px solid ${edit ? "#99bb99" : "#bbb"}`,
            background: edit ? "#ddeedd" : "#f5f5f5",
            cursor: "pointer",
            color: edit ? "#557755" : "#666",
            fontWeight: edit ? 700 : 400,
          }}
        >
          {edit ? "✓ Ver" : "✏️ Editar"}
        </button>
        <button
          onClick={() => setConLetra((v) => !v)}
          style={{
            fontSize: 9,
            padding: "3px 12px",
            borderRadius: 4,
            border: `1.5px solid ${conLetra ? "#ccaaaa" : "#bbb"}`,
            background: conLetra ? "#f2e0e0" : "#f5f5f5",
            cursor: "pointer",
            color: conLetra ? "#996666" : "#666",
            fontWeight: conLetra ? 700 : 400,
          }}
        >
          {conLetra ? "🎵 Sin letra" : "📝 Con letra"}
        </button>
        {[
          {
            lbl: "Transp.",
            val: semi === 0 ? "0" : semi <= 6 ? `+${semi}` : `−${12 - semi}`,
            mn: () => btnSemi(-1),
            pl: () => btnSemi(+1),
            rst:
              semi !== 0
          ...
          },
          {
            lbl: `Capo ${capo}°`,
            val: "",
            mn: () => setCapo((c) => Math.max(0, c - 1)),
            pl: () => setCapo((c) => Math.min(7, c + 1)),
            rst: capo > 0 ? () => setCapo(0) : null,
          },
          {
            lbl: "Sección",
            val: "",
            mn: () => remS(data.secciones.length - 1),
            pl: addS,
            rst: null,
          },
          {
            lbl: `Col.(${cols})`,
            val: "",
            mn: () => setCols((c) => Math.max(1, c - 1)),
            pl: () => setCols((c) => Math.min(8, c + 1)),
            rst: null,
          },
          {
            lbl: "Zoom",
            val: `${Math.round(zoom * 100)}%`,
            mn: () => setZoom((z) => Math.max(0.6, +(z - 0.1).toFixed(1))),
            pl: () => setZoom((z) => Math.min(1.8, +(z + 0.1).toFixed(1))),
            rst: zoom !== 1 ? () => setZoom(1) : null,
          },
      ...
      {data.secciones.map((sec, si) => {
        ...
        const fs =
          lbl.length > 0
            ? Math.min(9, Math.max(5, Math.floor((totalH / lbl.length) * 1.8)))
            : 7;
        const rows = [];
          ...
          >
            ...
            >
              {edit ? (
                <select
                  value={sec.label}
                  onChange={(e) => updL(si, e.target.value)}
                  style={{
                ...
                >
                  {["INTRO", "ESTROFA", "ESTRIBILLO", "PUENTE", "OUTRO"].map(
                    (l) => (
              ...
              {rows.map((row, ri) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      display: "flex",
                      borderTop: ri > 0 ? `1px solid ${c.bd}33` : "none",
                      minHeight: edit
                        ? conLetra
                          ? 64 * zoom
                          : 51 * zoom
                    ...
                    {padded.map((item, ci) => {
                      ...
                      return edit ? (
                        <CompasEdit
                          ...
                          b={b}
                          conLetra={conLetra}
                          onChange={(nc) => updC(si, item.idx, nc)}
                          onFocus={() => setFocus({ si, ci: item.idx })}
                          isFocused={focus?.si === si && focus?.ci === item.idx}
                          zoom={zoom}
                        />
                      ) : (
                        <CompasView
                ...
                <div style={{ display: "flex" }}>
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ color: "#d44", cursor: "pointer" }}
                    >
                      −
                    </span>
                  </button>
